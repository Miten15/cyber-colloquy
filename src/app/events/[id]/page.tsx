...
export default function EventDetailsPage() {
  ...
  const timelineEvents: TimelineEvent[] = ["21st March", "22nd March", "23rd March", "24th March"].map((day) => {
    const dayActivities = event.activities?.filter(activity => activity.date === day) || [];
    const allActivitiesDescription = dayActivities
      .map(activity => activity.description)
      .join(", ");

    return {
      date: day,
      title: `Day ${day.split(" ")[0]}`,
      description: allActivitiesDescription || "No activities scheduled",
      status: new Date(day + ", 2024") < new Date() ? "completed" : 
             new Date(day + ", 2024").toDateString() === new Date().toDateString() ? "current" : 
             "upcoming"
    };
  });
  ...
  return (
    <Container>
      ...
      {/* Timeline Section */}
      <div className="mb-12">
        <h3 className="text-2xl font-semibold mb-6 text-center">Event Timeline</h3>
